      ...
      {imgSrc ? (
        <div className="relative w-full h-full">
          ...
          {/* Theme-tinted overlay so background adapts to selected color.
              In dark mode we avoid using the theme gradient overlay (prevents
              the illustration from inheriting/showing theme colors). */}
          {dark ? (
            <div className="absolute inset-0 bg-black/40" />
          ) : (
            <div className={`absolute inset-0 ${classes.headerBar} opacity-10 mix-blend-multiply`} />
          )}
